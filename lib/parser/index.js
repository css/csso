...
function getInfo(idx) {
    ...
    // make copy of info since it could be reused by different tokens
    return {
        offset: info.offset,
        line: info.line,
        column: info.column
...
function getAtrulerq() {
    return (needInfo? [getInfo(pos), CSSPNodeType.AtrulerqType] : [CSSPNodeType.AtrulerqType]).concat(getTsets());
}
...
function getAtrulers() {
    var atrulers = (needInfo? [getInfo(pos), CSSPNodeType.AtrulersType] : [CSSPNodeType.AtrulersType]).concat(getSC());
...
function getAtrules() {
    var atrules = (needInfo? [getInfo(pos), CSSPNodeType.AtrulesType, getAtkeyword()] : [CSSPNodeType.AtrulesType, getAtkeyword()]).concat(getTsets());
...
function getClazz() {
    ...
    return needInfo?
            [getInfo(startPos), CSSPNodeType.ClazzType, getIdent()] :
...
function getComment() {
    ...
    return needInfo?
            [getInfo(startPos), CSSPNodeType.CommentType, s] :
...
function getProgid() {
    ...
    return (needInfo? [getInfo(startPos), CSSPNodeType.ProgidType] : [CSSPNodeType.ProgidType])
            .concat(getSC())
...
function getProperty() {
    ...
    return (needInfo?
            [getInfo(startPos), CSSPNodeType.PropertyType, getIdent()] :
...
function getPseudoe() {
    ...
    return needInfo?
            [getInfo(startPos), CSSPNodeType.PseudoeType, getIdent()] :
...
function getPseudoc() {
    ...
    return needInfo?
            [getInfo(startPos), CSSPNodeType.PseudocType, checkFunktion(pos)? getFunktion() : getIdent()] :
...
function getString() {
    ...
    return needInfo?
            [getInfo(startPos), CSSPNodeType.StringType, tokens[pos++].value] :
...
function getUnary() {
    ...
    return needInfo?
            [getInfo(startPos), CSSPNodeType.UnaryType, tokens[pos++].value] :
...
function getUnknown() {
    ...
    return needInfo?
            [getInfo(startPos), CSSPNodeType.UnknownType, tokens[pos++].value] :
...
function getUri() {
    ...
    if (checkUri1(pos)) {
        var uri = (needInfo? [getInfo(startPos), CSSPNodeType.UriType] : [CSSPNodeType.UriType])
                    .concat(getSC())
                    .concat([getString()])
                    .concat(getSC());
