...
function checkBlockdecl(_i) {
    ...
    if (l = _checkBlockdecl0(_i)) tokens[_i].bd_type = _getBlockdecl0;
    else if (l = _checkBlockdecl1(_i)) tokens[_i].bd_type = _getBlockdecl1;
    else if (l = _checkBlockdecl2(_i)) tokens[_i].bd_type = _getBlockdecl2;
    else if (l = _checkBlockdecl3(_i)) tokens[_i].bd_type = _getBlockdecl3;
    else return fail(tokens[_i]);
...
function getDeclaration() {
    var declaration = [getInfo(pos), NodeType.DeclarationType, getProperty()];
...
function getDecldelim() {
    ...
    return [getInfo(startPos), NodeType.DecldelimType];
}
...
function checkValue(_i) {
    ...
    while (_i < tokens.length) {
        ...
        // else if (len = checkBlock(_i)) getter = getBlock;
        // else if (len = checkAtkeyword(_i)) getter = getAtkeyword;
