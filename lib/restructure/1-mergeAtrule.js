const { List, walk, keyword: resolveKeyword } = require('css-tree');
...
function relocateAtrules(ast, options) {
    ...
    ast.children.each((node, item, list) => {
        ...
        } else if (topInjectPoint === null) {
            topInjectPoint = item;
        }
    });

    for (const atrule in collected) {
        if (hasOwnProperty.call(collected, atrule)) {
            for (const id in collected[atrule]) {
                if (hasOwnProperty.call(collected[atrule], id)) {
