const { List, generate, walk } = require('css-tree');
...
class TRBL {
    ...
    getValueSequence(declaration, count) {
        const values = [];
        let iehack = '';
        const hasBadValues = declaration.value.children.forEach(child => {
            let special = false;
...
function processRule(rule, shorts, shortDeclarations, lastShortSelector) {
    ...
    rule.block.children.eachRight((declaration, item) => {
        const { property } = declaration;

        if (!hasOwnProperty.call(MAIN_PROPERTY, property)) {
            return;
        }
...
function processShorthands(shortDeclarations, markDeclaration) {
    shortDeclarations.forEach(item => {
