...
function showStat(filename, source, result, map, time, mem) {
    function fmt(size) {
...
var command = cli.create('csso', '[input] [output]')
    ...
    .option('-m, --map <filename>', 'Output source map file')
    .option('--inline-map', 'Add inlined source map to result CSS')
    .option('--input-map <filename>', 'Input source map file')
    ...
    .action(function(args) {
        ...
        var mapFile = this.values.map;
        var inputMapFile = this.values.inputMap;
        ...
        if (mapFile) {
            if (!fs.existsSync(path.dirname(mapFile))) {
                console.error('Directory for map file should exists:', path.dirname(path.resolve(mapFile)));
                process.exit(2);
            }
        ...
        var inlineMap = this.values.inlineMap;
        var input = inputFile === '<stdin>' ? process.stdin : fs.createReadStream(inputFile);
        var inputSourceMap = null;

        if (inputMapFile) {
            inputSourceMap = fs.readFileSync(inputMapFile, 'utf8');
        }
        ...
        readFromStream(input, function(source) {
            ...
            try {
                result = csso.minify(source, {
                    ...
                    sourceMap: inlineMap ? 'inline' : Boolean(mapFile),
            ...
            if (mapFile) {
                fs.writeFileSync(mapFile, result.map.toString(), 'utf-8');
                result.css += '\n/*# sourceMappingURL=' +
                    path.relative(outputFile || process.cwd(), mapFile) +
                    ' */';
            }
