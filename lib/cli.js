...
var command = cli.create('csso', '[input] [output]')
    ...
    .option('-m, --map <target>', 'Generate source map. Possible values: "inline", "file" or filename')
    ...
    .action(function(args) {
        ...
        readFromStream(input, function(source) {
            ...
            if (mapFile && mapFile !== 'inline') {
                fs.writeFileSync(mapFile, result.map.toString(), 'utf-8');
                result.css += '\n/*# sourceMappingURL=' +
                    path.relative(outputFile ? path.dirname(outputFile) : process.cwd(), mapFile) +
                    ' */';
            }
