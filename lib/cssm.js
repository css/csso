var $cssm = {
    ...
    processComments: function(nodes) {
        var node, i, j, c, t, mode = { protect: false, order: false };
    ...
    // TODO: review it and make it faster
    restructure: function(nodes) {
        ...
        for (i in indice) {
            ...
            if (node.type === 'ruleset' || (node.type === 'atrule' &&
                    (node.value[1] === 'font-face' || node.value[1] === 'page'))) {
        ...
        for (i = 0; i < nodes.length; i++) {
            ...
            else if (node.type === 'ruleset' || (node.type === 'atrule' &&
                    (node.value[1] === 'font-face' || node.value[1] === 'page'))) {
    ...
    collapseShorthands: function(content) {
        ...
        function _collapseShorthand(name, important) {
            var i, d, v = [null, null, null, null], hasTRBL = false, n = -1000000, t;
