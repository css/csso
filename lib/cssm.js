var $cssm = {
    ...
    divideContent: function(content, indice) {
        var r = { m: [], u: [] },
            i, d, l,
            bi = 0,
            bl = content[0].key.length * indice[content[0].key];
        
        for (i = 0; i < content.length; i++) {
            d = content[i];
            if (indice[d.key] > 1) r.m.push(d);
            else r.u.push(d);
        }
        ...
        if (!r.u.length) {
            for (i = 1; i < content.length; i++) {
                d = content[i];
                if ((l = d.key.length * indice[content[i].key]) > bl) {
                    bi = i;
                    bl = l;
                }
            }
            r.u.push(r.m.splice(bi, 1)[0]);
        }
    ...
    possibleJoin: function(nodes, indice) {
        ...
        for (i = 0; i < indice.length; i++) {
            node = nodes[indice[i]];
            if (node.content.length > 1) r.push(indice[i]);
        }
