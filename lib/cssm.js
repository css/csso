    ...
    src.nodes.forEach(function(node) {
        if (node.type === 'ruleset' || (node.type === 'atrule' && (node.value[1] === 'page' || node.value[1] === 'font-face'))) {
            if (node.value.length && node.content.length) {
                ...
                node.content.forEach(function(d) {
                    if (d.name && d.value.length) t.content.push({ name: d.name, value: d.value.slice() });
                });
                r.push(t);
            }
        } else {
            switch (node.value[1]) {
                ...
                case 'media':
                    if (node.nodes.length) {
                        t = { type: 'atrule', value: node.value.slice(), content: [], nodes: [] };
                        r.push(clean_copy(node.nodes, t));
                    }
                    break;
            }
        }
...
function restructure(nodes) {
    ...
    for (skey in sstore) {
        ...
        for (t in sso.d) {
            dkey = t + ':' + sso.d[t].join();
            rdstore[dkey] = { name: t, value: sso.d[t] }; // store tokens
