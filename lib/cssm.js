...
//        nodes = $cssm.safe(nodes);
    ...
    safe: function(nodes) {
        var i, j, k, t, node, indice = {}, s, bc, tn, chaos = [], d;
        ...
        for (i = nodes.length - 1; i !== -1; i--) {
            node = nodes[i];
            if (node.content.length) {
                if ($cssm.prepareNode($cssm.joinValue(node.value), node.content, node.type === 'ruleset' ? indice : null)) {
                    node.content = $cssm.collapseShorthands(node.content);
                } else nodes.splice(i, 1);
            }
        }

        $cssm.joinAllEqualNodes(nodes);
    ...
    todo_mergeNodes: function(nodes) {
        var i = 0, j, node;
        ...
        for (; i < nodes.length; i++) {
            ...
            if (node.type === 'ruleset') {
                ...
                if (j !== -1) {
                    t = nodes[j];
                    if ($cssm.joinValue(node.value, true) === $cssm.joinValue(t.value, true)) {
                        node.content = node.content.concat(t.content);
                        nodes.splice(j, 1);
                        i--;
                    } else {
                        a1 = node.content.slice();
                        a2 = t.content.slice();
                        if (equal(a1, a2)) {
                            node.content = join(a1, a2);
                            node.value = node.value.concat(t.value);
                            nodes.splice(j, 1);
                            i--;
                        }
                    }
    ...
    tryToJoinNodes: function(nodes, i, j) {
        ...
        function resort(a1, a2) {
            function findSame(a, key) {
            ...
            return (sort(a1, a2) && sort(a2, a1));
        }

        function equal(a1, a2) {
            return !resort(a1, a2) ? false : $cssm.isOrderCorrect(a1) && $cssm.isOrderCorrect(a2);
        }
        ...
        if ($cssm.joinValue(node1.value, true) === $cssm.joinValue(node2.value, true)) {
            ...
            a1 = node1.content.slice();
            a2 = node2.content.slice();
            if (equal(a1, a2)) {
                node1.content = join(a1, a2);
    ...
    joinAllEqualNodes: function(nodes) {
        var i = 0, j, node;

        for (; i < nodes.length; i++) {
            node = nodes[i];
            if (node.type === 'ruleset') {
                if ((j = $cssm.nextNode(nodes, 'ruleset', i + 1)) !== -1 && $cssm.tryToJoinNodes(nodes, i, j)) i--;
            }
        }
    },
        ...
        function _collapseShorthand(name) {
            ...
            if (hasTRBL) {
                if (v[0] && v[1] && v[2] && v[3]) {
                    ...
                    if (allowBase) r.push({ name: [name], value: t, important: imp.n === 4, n: n });

                    if (impMode) for (i in sh) if (sh[i].important || !allowBase) r.push(sh[i]);
                } else {
                    for (i in names[name]) {
                        t = names[name][i];
                        if (v[t]) r.push({ name: [i], value: [v[t]], important: imp.names[i] === true, n: n });
                    }
                }
            }
        }
