...
var $cssm = {
    ...
    safe: function(nodes) {
        ...
        for (i = nodes.length - 1; i !== -1; i--) {
            node = nodes[i];
            if (node.type === 'ruleset') {
                node.value = $cssm.cleanValue(node.value);
...
//              nodes = [].concat(nodes.slice(0, i), $cssm.expandNode(node), nodes.slice(i + 1));
        ...
        do {
            ...
            for (i = 0; i < nodes.length; i++) {
                if ((cNode = nodes[i]).type === 'ruleset') {
                    nNode = ((j = $cssm.nextNode(nodes, 'ruleset', i + 1)) !== -1 ? nodes[j] : null);
                    if (nNode) {
                        t = $cssm.tryToDisjoinNodes(cNode, nNode);
                        if (t && t.c2.length) {
                            cNode.content = t.c1;
                            nNode.content = t.c3;
                            t = { type: 'ruleset', value: $cssm.cleanValue(cNode.value.concat(nNode.value)), content: t.c2, nodes: [] };
                            nodes.splice(i + 1, 0, t);
                            onceAgain = true;
                        }
                    }
                }
    ...
    tryToJoinNodes: function(nodes, i, j) {
        ...
        if ($cssm.joinValue(node1.value, true) === $cssm.joinValue(node2.value, true)) {
            ...
            if ($cssm.equalContent(c1, c2)) {
                ...
                node1.value = $cssm.cleanValue(node1.value.concat(node2.value));
                nodes.splice(j, 1);
                return true;
            }
        }
    ...
    // TODO: it is needed to disjoin ordered declarations also
    tryToDisjoinNodes: function(node1, node2) {
        var r = { c1: node1.content.slice(), c2: [], c3: node2.content.slice() }, i = 0, d, j,
            nl = $cssm.joinValue(node1.value).length + $cssm.joinValue(node2.value).length;

        for (; i < r.c1.length; i++) {
            d = r.c1[i];
            if (d.n > 0) return null;
            if ((j = $cssm.findSame(r.c3, d.key)) !== -1) {
                r.c2.push(r.c1.splice(i--, 1)[0]);
                r.c3.splice(j, 1);
            }
        }

        return r.c2.length && (nl < $cssm.getContentLength(r.c2)) ? r : null;
    },
