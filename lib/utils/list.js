...
var protectedItems = new WeakSet();
...
List.prototype.each = function(fn, context) {
    ...
    while (cursor) {
        protectedItems.add(cursor);

        var res = fn.call(context || this, cursor.data, cursor);

        next = cursor.next;
        protectedItems.delete(cursor);
...
List.prototype.remove = function(item) {
    if (protectedItems.has(item)) {
        throw new Error('item can\'t to be remove from list, since has cursor on it');
    }
