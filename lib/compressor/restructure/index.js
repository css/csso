...
module.exports = function(ast, debug) {
    ...
    var declarationMarker = (function() {
        ...
        return function markDeclaration(node) {
            ...
            node.info.s = names.resolve(property) + (values.resolve(value) << 12);
            node.info.length = property.length + 1 + value.length;
    ...
    walkRulesetsRight('buildMaps', function(ruleset, stylesheet) {
        var map = maps.get(stylesheet);
        if (!map) {
            map = new Map();
            maps.set(stylesheet, map);
        ...
        var rulesetId = (ruleset.info.pseudoSignature || '') + '|' + ruleset.selector.selectors.first().info.s;
