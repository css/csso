...
function getHash(list) {
    return new Set(list.map(function(data) {
        return data.info.s;
    }));
}
...
function compareRulesets(ruleset1, ruleset2) {
    ...
    items1.each(function(data) {
        if (data.info.fingerprint) {
            fingerprints.add(data.info.fingerprint);
        }

        if (hash2.has(data.info.s)) {
            result.eq.push(data);
        } else {
    ...
    items2.each(function(data) {
        if (!hash1.has(data.info.s)) {
            // if ruleset1 has overriding declaration, this is not a difference
            if (!fingerprints.has(data.info.fingerprint)) {
