...
module.exports = function restructureRuleset(node, parent, array, i) {
    ...
    for (i = i - 1; i >= 0; i--) {
        ...
        var prevHash = utils.getSelectorHash(prevSelector);
        var hash = utils.getSelectorHash(selector);
            ...
            if (!diff.ne1.length && !diff.ne2.length) {
                ...
                if (diff.ne1.length && !diff.ne2.length) {
                    ...
                    if (selectorLength < blockLength) {
                        utils.addToSelector(prevSelector, selector.copy());
                        node.block = {
                            type: 'Block',
                            info: block.info,
                    ...
                    if (selectorLength < blockLength) {
                        utils.addToSelector(selector, prevSelector.copy());
                        prevNode.block = {
                            type: 'Block',
                            info: prevBlock.info,
                    ...
                    var newSelector = {
                        type: 'Selector',
                        info: {},
                        selectors: utils.addToSelector(prevSelector.copy(), selector.copy())
                    };
