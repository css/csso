...
module.exports = function rejoinRuleset(node, parent, array, i) {
    ...
    for (i = i - 1; i >= 0; i--) {
        ...
        while (prevSelectorCursor) {
            var selectorCursor = selector.head;

            while (selectorCursor) {
                if (prevSelectorCursor.data.info.compareMarker === selectorCursor.data.info.compareMarker) {
                    return;
                }

                selectorCursor = selectorCursor.next;
            }
