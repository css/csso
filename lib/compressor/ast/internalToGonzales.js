...
function buildArguments(body, args) {
    args.each(function(data, item) {
        body.push.apply(body, data.sequence.toArray().map(toGonzales));
        if (item.next) {
            body.push([{}, 'operator', ',']);
        }
    });
            ...
            if (node.expression && !node.expression.sequence.isEmpty()) {
                if (type === 'atruler') {
                    result.push([
                        ...
                        'atrulerq',
                        [{}, 's', ' ']
                    ].concat(node.expression.sequence.toArray().map(toGonzales)));
                } else {
                    result.push([{}, 's', ' ']);
                    result = result.concat(node.expression.sequence.toArray().map(toGonzales));
                }
            } else {
                if (type === 'atruler') {
                    result.push([
        ...
        case 'Value':
            var result = [
                ...
                node.sequence.first().type === 'Progid'
                    ? 'filterv'
                    : 'value'
            ].concat(node.sequence.toArray().map(toGonzales));
