...
function listEachDelim(node, type, itemsProperty, delimeter) {
    var result = [node.info, type];
...
function toGonzales(node) {
    switch (node.type) {
        ...
        case 'Selector':
            return listEachDelim(node, 'selector', 'selectors', [{}, 'delim']);
        ...
        case 'SimpleSelector':
            ...
            node.sequence.each(function(data) {
                ...
                if (node[1] === 'ident' && /\|/.test(node[2])) {
                    result.push(
                        [{}, 'ident', node[2].split('|')[0]],
                        [{}, 'namespace'],
                        [{}, 'ident', node[2].split('|')[1]]
                    );
                } else {
                    result.push(node);
                }
        ...
        case 'Negation':
            var body = listEachDelim(node, 'functionBody', 'sequence', [{}, 'delim']);
