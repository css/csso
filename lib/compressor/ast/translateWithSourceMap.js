...
function generateSourceMap(root, source) {
...
function translate(node) {
    switch (node.type) {
        ...
        case 'SimpleSelector':
            return createSourceNode(node.info, node.sequence.map(translate));
...
module.exports = function(node, options) {
    ...
    var inlineMap = '';
    var result = generateSourceMap(new SourceNode(null, null, null, translate(node)), filename);
    ...
    if (options.inputSourceMap) {
        ...
        if (typeof options.source === 'string') {
            result.map.setSourceContent(filename, options.source);
        }
    }
