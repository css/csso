...
function listEachDelim(list, buffer, delimeter) {
    list.each(function(data, item) {
        translate(data, buffer);
...
function translate(node, buffer) {
    switch (node.type) {
        case 'Atrule':
        ...
        case 'FunctionalPseudo':
            buffer.push(':', node.name, '(');
            listEachDelim(node.arguments, buffer, ',');
            buffer.push(')');
            break;
        ...
        case 'Selector':
            listEachDelim(node.selectors, buffer, ',');
            break;

        case 'Negation':
            buffer.push(':not(');
            listEachDelim(node.sequence, buffer, ',');
            buffer.push(')');
            break;
