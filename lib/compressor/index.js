...
CSSOCompressor.prototype.compress = function(ast, structureOptimizations) {
    this.init();
    ...
    ast = this.walk(rules.cleanComments, ast, '/0');
    ast = this.walk(rules.compress, ast, '/0');
    ast = this.walk(rules.prepare, ast, '/0');
    ast = this.walk(rules.freezeRuleset, ast, '/0');

    // restructure ON
    if (structureOptimizations) {
        var initAst = this.copyArray(ast);
        var initLength = translate(initAst, true).length;

        ast = this.walk(rules.rejoinRuleset, ast, '/0');
        this.disjoin(ast);
        ast = this.walk(rules.markShorthand, ast, '/0');
        ast = this.walk(rules.cleanShortcut, ast, '/0');
        ast = this.walk(rules.restructureBlock, ast, '/0');
        ...
        do {
            ...
            ast = this.walk(rules.rejoinRuleset, ast, '/0');
            ast = this.walk(rules.restructureRuleset, ast, '/0');
...
CSSOCompressor.prototype.walk = function(rules, token, path) {
    for (var i = token.length - 1; i >= 2; i--) {
        var child = token[i];
...
CSSOCompressor.prototype.freezeRulesets = function(token, rule) {
    var info = token[0];
...
CSSOCompressor.prototype.compressFunctionColor = function(token, rule, parent, idx) {
    ...
    if (token[2][2] === 'rgb') {
        ...
        if (args.length === 3) {
            var color = toHex(args[0][2]) + toHex(args[1][2]) + toHex(args[2][2]);

            // TODO: normalize numbers, i.e. <0 -> 0, >255 -> 255
            if (color.length !== 6) {
                return;
            }
...
CSSOCompressor.prototype.compressBackground = function(token) {
    ...
    var i, x, t,
        n = value[value.length - 1][1] === 'important' ? 3 : 2;
...
module.exports = function(tree, noStructureOptimizations) {
    return new CSSOCompressor().compress(tree, !noStructureOptimizations);
};
