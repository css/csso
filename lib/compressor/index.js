...
function compressBlock(ast, restructuring, num) {
    ...
    if (restructuring) {
        walk('prepare', root, require('./prepare/index.js'));
...
function disjoin(token, parent, i) {
...
function markShorthands(ruleset, parent) {
    var token = ruleset[3];
    var selector = ruleset[2][2][0].s;
...
function restructureBlock(ruleset, parent) {
...
function getVendorIDFromToken(token) {
    ...
    if (name) {
        return constants.vendorID[name] || '';
    }
...
function getPropertyInfo(name) {
    ...
    return propertyInfoMap[name] = {
        ...
        hack: hack,
        vendor: vendor,
        table: constants.nlTable[name.substr(vendor.length)]
    };
}
...
function rejoinAtrule(token, parent, i) {
...
function rejoinRuleset(token, parent, i) {
    var selector = token[2];
    var block = token[3];
    var info = token[0];
...
function restructureRuleset(token, parent, i) {
    var selector = token[2];
    var block = token[3];
    var info = token[0];
    ...
    for (i = i - 1; i >= 2; i--) {
        ...
        // console.log(translate(selector, true), translate(prevSelector, true));
        ...
        if (diff.eq.length) {
            if (!diff.ne1.length && !diff.ne2.length) {
                ...
                if (diff.ne1.length && !diff.ne2.length) {
                    ...
                    // extract equal block
                    var newSelector = addToSelector(utils.copyArray(prevSelector), selector);
