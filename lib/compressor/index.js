...
CSSOCompressor.prototype.rejoinRuleset = function(token, rule, container, i) {
    ...
    if (prevSelector.length > 2 &&
        ...
        if (utils.equalHash(hash, prevHash)) {
            prev[3] = prevBlock.concat(block.splice(2));
            return null;
        }
        ...
        if (this.okToJoinByProperties(token, prev)) {
            ...
            if (!r.ne1.length && !r.ne2.length) {
                ignore:
                for (var j = 2; j < selector.length; j++) {
                    var pos = prevSelector.length;
                    var simpleSelectorStr = selector[j][0].s;
                    while (pos > 2) {
                        var prevSimpleSelectorStr = prevSelector[pos - 1][0].s;
                    ...
                    prevSelector.splice(pos, 0, selector[j]);
                }
