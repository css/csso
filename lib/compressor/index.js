...
function debug_(name, token) {
    ...
    if (token) {
        ...
        // line += '\n  ' + (css.length > 128 ? css.substr(0, 128) + '...' : css) + '\n';
    }
...
function walkRulesets(stylesheets, fn) {
    var id = 0;
    stylesheets.forEach(function(stylesheet, id) {
        ...
        for (var i = stylesheet.length - 1; i >= 2; i--) {
            ...
            if (token[1] === 'ruleset') {
                var result = fn.call(this, token, '', stylesheet, i, '/0/' + id + '/' + i);
                if (result === null) {
...
function compressBlock(ast, restructuring, num) {
    ...
    if (restructuring) {
        ...
        walkRulesets.call(this, root.rulesetParents, this.rejoinRuleset);
        ...
        // walkRulesets.call(this, root.rulesetParents, this.markShorthands);
        // debug('markShorthand', ast);
        this.shortDeclarations.forEach(processShorthand);
        debug('optimizeShorthands', ast);
        ...
        do {
            ...
            walkRulesets.call(this, root.rulesetParents, this.restructureRuleset);
            debug('restructureRuleset', ast);
        } while (this.restructured);
    }
...
CSSOCompressor.prototype.rejoinRuleset = function(token, rule, parent, i) {
    var prev = i === 2 || parent[i - 1][1] === 'unknown' ? null : parent[i - 1];
