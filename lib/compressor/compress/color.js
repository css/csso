    ...
    body.each(function(node) {
        ...
        switch (type) {
            ...
            case 'percentage':
                ...
                args.push({
                    type: type,
                    value: Number(type === 'number' ? node.value : node.value.value)
                });
                break;
...
function compressFunction(node) {
    ...
    if (functionName === 'rgba' || functionName === 'hsla') {
        ...
        if (functionName === 'hsla') {
            ...
            node.name.value = 'rgba';
        }

        if (args[3] !== 1) {
            // replace argument values for normalized/interpolated
            node.body.each(function(node, idx) {
                var type = node.type;

                if (type === 'number' || type === 'percentage') {
                    node.replace([{}, 'number', packNumber(args.shift())]);
    ...
    if (functionName === 'rgb') {
        ...
        compressHex(node.replace([node.info, 'vhash', color]));
    }
}

function compressIdent(node) {
    ...
    if (hex) {
        if (hex.length + 1 <= color.length) {
            ...
            node.replace([node.info, 'vhash', hex]);
        } else {
...
function compressHex(node) {
    ...
    if (HEX_TO_NAME[color]) {
        node.replace([node.info, 'ident', HEX_TO_NAME[color]]);
    } else {
        node.value = color;
    }
}
