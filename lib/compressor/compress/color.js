...
function compressFunction(node, parent, list, item) {
    ...
    if (functionName === 'rgb') {
        ...
        if (Array.isArray(list)) {
            var next = list[item + 1];
            if (next && next.type !== 'Space') {
                list.splice(item + 1, 0, {
                    type: 'Space'
                });
            }
        } else {
        ...
        compressHex(item.data, null, null, item);
    }
}

function compressIdent(node, parent, list, item) {
...
function compressHex(node, parent, list, item) {
